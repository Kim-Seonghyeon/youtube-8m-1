...
if __name__ == '__main__':
  flags.DEFINE_string("train_dir", "/home/ksh/youtube/data/frame_level/train/",
                      "The directory to load the model files from.")
  flags.DEFINE_string("output_file", "/home/ksh/youtube/data/frame_level_cpr/train/",
                      "The file to save the predictions to.")
  flags.DEFINE_string(
      "input_data_pattern", "/home/ksh/youtube/data/frame_level/train/train*",
...
def inference(reader, train_dir, data_pattern, out_file_location, batch_size, top_k):
  with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    ...
    logging.info(num_frames_batch)
    num_frames_batch = tf.expand_dims(num_frames_batch,1)
    logging.info(num_frames_batch)
    video_batch_30 = model_utils.EqualSpaceMeans(video_batch, num_frames_batch, 30)
    ...
    try:
      while not coord.should_stop():
        ...
        for i in range(8):
          video_batch_30_val, video_id_batch_val, video_batch_val,num_frames_batch_val = sess.run([video_batch_30, video_id_batch, video_batch, num_frames_batch])
        ...
        video_batch_30_val_tot = np.concatenate(video_batch_30_val_tot, axis=0)
        video_id_batch_val_tot = np.concatenate(video_id_batch_val_tot, axis=0)
        logging.info(video_batch_30_val_tot.shape)
        with gfile.Open(out_file_location + 'id/' + 'id_' + str(file_num)+'.npy', "w+") as out_file:
          np.save(out_file, video_id_batch_val_tot)
        with gfile.Open(out_file_location + 'feature/' + 'feature_' + str(file_num)+'.npy', "w+") as out_file:
          np.save(out_file, video_batch_30_val_tot)
        file_num+=1
