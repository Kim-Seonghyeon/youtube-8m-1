...
from keras.layers import Dense, Dropout, Activation, Conv2D, Reshape
...
model.add(Dense(256, input_dim=width * height, init='glorot_uniform', activation='relu', kernel_regularizer=l1_reg))
...
def l1_reg(weight_matrix):
    weight_matrix = K.reshape(weight_matrix,[28,28,-1])
    return 0.000001 * (K.sum(K.abs(weight_matrix[1:,:]-weight_matrix[:-1,:])) + K.sum(K.abs(weight_matrix[:,1:]-weight_matrix[:,:-1])) +
                   K.sum(K.abs(weight_matrix[1:, 1:] - weight_matrix[:-1, :-1])) + K.sum(K.abs(weight_matrix[:-1, 1:] - weight_matrix[1:, :-1]))) + 0.00000001 * K.sum(K.abs(weight_matrix))
...
# 첫 번째 Layer (Input layer)
model.add(Conv2D(30, (3,3), input_shape=(28, 28, 1), init='glorot_uniform', activation='relu'))
model.add(Dropout(0.3))

# 두 번째 Layer (Hidden layer 1)
# 두 번째 Layer부터는 output_dim만 설정하면 된다
# input_dim은 이전 레이어의 output_dim과 같다고 가정함
model.add(Conv2D(30, (3,3), init='glorot_uniform', activation='relu'))
model.add(Dropout(0.3))
# input_dim은 이전 레이어의 output_dim과 같다고 가정함
model.add(Conv2D(30, (3,3), init='glorot_uniform', activation='relu'))
model.add(Dropout(0.3))
model.add(Reshape((22*22*30,)))
